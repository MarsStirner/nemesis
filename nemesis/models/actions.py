...
from nemesis.models.exists import FDRecord
...
class Action(db.Model):
    ...
    def __getitem__(self, code):
        ...
        находится тип свойства всправчонике, то возникает ошибка.
        """
        if code in self.propsByCode:
            return self.propsByCode[code]
        else:
    ...
    def get_default_value(self):
        ...
        if self.type.isVector:
            try:
                ...
                logger.error(u'Ошибка парсинга JSON из defaultValue для apt.id = {0}'.format(self.type.id))
                return None

            if not isinstance(default_values, list):
                logger.error(u'JSON из defaultValue не является списком для apt.id = {0}'.format(self.type.id))
                return None

            result = []
            for value in default_values:
                tmp = value_container_class.get_default_val(value, self.type)
                tmp and result.append(tmp)
            return result
        else:
            return value_container_class.get_default_val(self.type.defaultValue, self.type)
...
class ActionProperty__ValueType(db.Model):
    ...
    @classmethod
    def get_default_val(cls, default_value, property_type):
...
class ActionProperty_ReferenceRb(ActionProperty_Integer_Base):
    ...
    def get_default_val(cls, default_value, property_type):
        ...
        table_name = property_type.valueDomain.split(';')[0]
